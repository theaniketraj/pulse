...
async function testPrometheusConnection() {
  ...
  try {
    const response = await makeRequest(`${CONFIG.prometheusUrl}/api/v1/status/config`);
    logTest(
    ...
    if (response.status === 200) {
      ...
      logTest('Prometheus API returns valid JSON', false, 'Server not responding correctly');
    }
  } catch (error) {
...
async function testMetricsEndpoints() {
  ...
  try {
    ...
    if (queryResponse.data && queryResponse.data.data) {
      logTest(
        ...
        queryResponse.data.data.result && Array.isArray(queryResponse.data.data.result),
  ...
  try {
    ...
    if (rangeResponse.data && rangeResponse.data.data) {
      const hasValues = rangeResponse.data.data.result?.some(r => r.values && r.values.length > 0);
      logTest(
    ...
    logTest('Query Range API endpoint (/api/v1/query_range)', false, error.message);
...
async function testTargetsEndpoint() {
  ...
  try {
    ...
    logTest(
      ...
      targetsResponse.status === 200 && targetsResponse.data.status === 'success',
...
async function testSpecificMetrics() {
  ...
  const metricsToTest = [
    ...
    { name: 'prometheus_build_info', description: 'Prometheus build information' },
    { name: 'prometheus_tsdb_head_samples_appended_total', description: 'TSDB samples' },
  ];
  ...
  for (const metric of metricsToTest) {
    try {
      ...
      const hasResults = response.data?.data?.result && response.data.data.result.length > 0;
      logTest(
        `Metric: ${metric.name}`,
        hasResults,
...
async function testDataProcessing() {
  ...
  try {
    ...
    if (response.data && response.data.data) {
      ...
      const hasSystemMetrics = metrics.includes('up') || metrics.includes('prometheus_build_info');
      logTest(
  ...
  try {
    ...
    if (response.data && response.data.data && response.data.data.alerts) {
      ...
      const validStates = alerts.every(
        alert => ['firing', 'pending', 'inactive'].includes(alert.state)
      );
      logTest(
