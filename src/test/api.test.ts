...
describe('PrometheusApi', () => {
    ...
    test('query handles API errors', async () => {
        ...
        await expect(api.query('bad_query')).rejects.toThrow('Prometheus API error: invalid query');
    });
    ...
    test('getAlerts fetches alerts correctly', async () => {
        ...
        expect(mockedAxios.get).toHaveBeenCalledWith('http://localhost:9090/api/v1/alerts');
        expect(result).toEqual(mockResponse.data);
    });
});
